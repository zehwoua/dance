 ...
 */
function siteStatistic(profileId) {
...
function drawVisualization(response) {
  if(response){
    var data = [];
    data.push( ['Date', 'Views']);
    for (var i = 0, row; row = response.rows[i]; ++i) {
      // var date = new Date( parseInt( row[0].substr(6) ) );
      var date = isoDateReviver(row[0]);
      data.push( [date, parseInt( row[2]) ] );
    }
